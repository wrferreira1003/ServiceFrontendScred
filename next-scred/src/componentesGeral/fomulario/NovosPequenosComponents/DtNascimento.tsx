...
export type CreateUserDataDtNascimento = zod.infer<typeof personalInfoSchema>;

interface FormularioSolicitacaoProps {
  handleFormDataChange: (data: CreateUserDataDtNascimento) => void;
  formData: CreateUserDataDtNascimento | undefined;
  setValidateAndSave: React.Dispatch<
    React.SetStateAction<(() => Promise<boolean>) | null>
  >;
}

export default function DataNascimentoComponent({
  formData,
  ...
  const {
    ...
    setValue,
    watch
  } = useForm<CreateUserDataDtNascimento>({
  ...
  async function createUser(data: CreateUserDataDtNascimento) {
    try {
